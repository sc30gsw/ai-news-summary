...
async function fetchFeed(feed: RSSFeed) {
  const result = await Result.tryPromise(async () => {
    ...
    return parsed.items.slice(ARRAY_START_INDEX, MAX_ITEMS_PER_FEED).map((item) => ({
      title: item.title || DEFAULT_TITLE,
      link: item.link || "",
      content: item.contentSnippet || item.content || "",
      pubDate: item.pubDate || format(now, DATE_FORMAT),
      feedName: feed.name,
    }));
  });
...
export async function fetchRSSFeeds() {
  ...
  const articlePromises = filteredItems
    .slice(ARRAY_START_INDEX, MAX_RSS_ARTICLES_TO_SUMMARIZE)
    .map((item) =>
      limit(async () => {
